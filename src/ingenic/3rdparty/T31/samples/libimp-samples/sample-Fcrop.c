...
/*
 * This example is used to test fcrop's image upscaling function
 * Where zoom_test array holds the magnification factor to be tested
 * Test on T31X, DDR 600M，ISP 200M，The maximum magnification is about 3.5x
 */
...
static void *res_get_video_stream(void *args)
{
	...
	float zoom_test[] = {1.5, 2, 2.5, 3, 3.5};
	for (i = 0; i < totalSaveCnt; i++) {
#if 1
		if ((chnNum == 0) && (i != 0) && (i % 30 == 0)) {  //Sample captures 200 frames by default
			int ret = -1;

			IMPISPFrontCrop fcrop, fcrop_get;
			memset(&fcrop_get, 0, sizeof(IMPISPFrontCrop));
			ret = IMP_ISP_Tuning_GetFrontCrop(&fcrop_get);
			printf("IMP_ISP_Tuning_GetFrontCrop ret = %d\n", ret);
			printf("fcrop.fcrop_enable = %d, fcrop.fcrop_height = %u, fcrop.fcrop_width = %u, fcrop.fcrop_left = %u, fcrop.fcrop_top = %u\n", 
				fcrop_get.fcrop_enable, fcrop_get.fcrop_height, fcrop_get.fcrop_width, fcrop_get.fcrop_left, fcrop_get.fcrop_top);

			fcrop.fcrop_enable = 1;
			fcrop.fcrop_height = ((unsigned int)(SENSOR_HEIGHT / zoom_test[i/30-1]) + 15) & (~15);
			printf("zoom: %f\n", zoom_test[i/30-1]);
			fcrop.fcrop_width = ((unsigned int)(SENSOR_WIDTH / zoom_test[i/30-1]) + 15) & (~15);
			fcrop.fcrop_left = 0;
			fcrop.fcrop_top = 0;
			ret = IMP_ISP_Tuning_SetFrontCrop(&fcrop);
			printf("IMP_ISP_Tuning_SetFrontCrop ret = %d\n", ret);
