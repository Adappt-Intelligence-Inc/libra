...
static void *IMP_Audio_Record_ALGO_AI_Thread(void *argv)
{
	...
	ret = IMP_AI_SetHpfCoFrequency(100);
	if(ret != 0) {
		printf("set ai hpf cut-off frequency error.\n");
		IMP_LOG_INFO(TAG, "set ai hpf cut-off frequency error.\n");
		return NULL;
	}
...
static void *IMP_Audio_Play_ALGO_AO_Thread(void *argv)
{
	...
	ret = IMP_AO_SetHpfCoFrequency(100);
	if(ret != 0) {
		printf("set ao hpf cut-off frequency error.\n");
		IMP_LOG_INFO(TAG, "set audio hpf cut-off frequency error.\n");
		return NULL;
	}
	ret = IMP_AO_EnableHpf(&attr);
	if(ret != 0) {
		printf("enable ao hpf error.\n");
		IMP_LOG_INFO(TAG, "enable audio hpf error.\n");
		return NULL;
	}
